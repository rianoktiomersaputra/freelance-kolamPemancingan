...
;(function ( $, window, document, undefined ) {
	...
	var multiField = function( elem, options ){
        ...
        "multiField": {
          "messages": {
            "removeConfirmation": "Yakin Akan Menghapus Obat?"
          }
        }
      };
	...
	multiField.prototype = {
      ...
      if(this.config.locale !== 'default'){
		...
		cloneSection : function() {
			...
			newChild.find('#nama_obat_add').each(function(){
				$(this).autocomplete({
				    source: "details.php",
				    select: function(event, ui) {
				      var el = ui.item;
				      // var resulte = "<p>label : " + el.label + " - id : " + el.id + "</p>";
				      // $("#resulte").append(resulte);
				      $("#id_obat").val(el.id);
				    }
				});
				// $('#nama_obat:last').autocomplete();
			});
